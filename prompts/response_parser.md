...
3. First extract universal infographic primitives from that semantic pass:
...
4. Detect the best layout intent for the universal structure:
...
5. Detect the strongest legacy structure family only when the response exactly needs a specialized renderer:
...
6. Detect ordered steps when a process exists.
7. Detect decision conditions, failure, exception, retry, and return branches when a flowchart is appropriate.
8. Detect sections, layers, zones, reusable principles, modules, and clickable drill-down topics when an information map is appropriate.
9. Remove explanatory filler.
10. Output normalized candidate fragments for the Infographic DSL Builder.
...
For architecture responses, do not treat the template as the information structure. First identify the actual layers, components, responsibilities, and data/control paths from the response. Use `dark_layered_architecture` only after the semantic pass shows a layered AI/IoT architecture with component groups and directional flows.
